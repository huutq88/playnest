...
import { RuleSpec, ConditionSpec, ActionSpec } from '@/types/level';
  ...
  private evaluateCondition(condition: ConditionSpec, sourceId: string, extraParams: Record<string, any>): boolean {
    switch (condition.type) {
      case 'overlap': {
        ...
        // Check distance threshold fallback (e.g. 80px)
  ...
  private executeActions(actions: ActionSpec[]): void {
    actions.forEach((action) => {
      switch (action.type) {
        ...
        case 'hideObject': {
          const targetObj = action.target ? this.objectsMap.get(action.target) : undefined;
          if (targetObj) targetObj.setVisible(false);
          break;
        }

        case 'showObject': {
          const targetObj = action.target ? this.objectsMap.get(action.target) : undefined;
          if (targetObj) targetObj.setVisible(true);
          break;
        }
